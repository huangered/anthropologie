...
Venda.Ebiz.AttributeSwatch.displayListAttributes = function (attrColumn, attrName){
	...
	for(var i=0; i < this.ListAttributes[attrColumn].length; i++){
		...
		if (attrName.toLowerCase() == "colour" || attrName.toLowerCase() == "color" || attrName.toLowerCase() == "currency") {
...
Venda.Ebiz.AttributeSwatch.updateMessage = function(){
	if(this.attrNum==1){
		...
		if(this.filters["att1"]=="" || this.filters["att1"]==undefined){
			document.getElementById("alertmessage").innerHTML = "<span class=notselectedmsg>Please select a " + this.attrDisplayName[0] + "</span> <span class=labelAttr>" + this.attrDisplayName[1] + ":</span> <span class=sizeselected>"+ this.filters['att2'] + "</span>";
		}else if(this.filters["att2"]=="" || this.filters["att2"]==undefined){
			document.getElementById("alertmessage").innerHTML = "<span class=labelAttr>" + this.attrDisplayName[0] + ":</span> <span class=colorselected>" + this.filters['att1'] + "</span> <span class=notselectedmsg>Please select a " + this.attrDisplayName[1] + "</span>";
		}
		...
		for (var eachAttrSet in product.attributeValues) {
			if(product.attributeValues[eachAttrSet].values["att1"]==this.filters["att1"]){
				if (product.attributeValues[eachAttrSet].data["atrsell"]!="" && product.attributeValues[eachAttrSet].data["atrsell"]!=undefined) {
					price = product.attributeValues[eachAttrSet].data["atrsell"].toFixed(2);
				}
				if (product.attributeValues[eachAttrSet].data["atrwas"]!="" && product.attributeValues[eachAttrSet].data["atrwas"]!=undefined){
					wasprice = product.attributeValues[eachAttrSet].data["atrwas"].toFixed(2);
				}
			}
		}	
...
Venda.Ebiz.AttributeSwatch.backorderbt="bt_backorder.gif";
			...
			if(product.attributeValues[eachAttrSet].values["att1"]==this.filters["att1"]){
				if (product.attributeValues[eachAttrSet].data["atretady"]!="" && product.attributeValues[eachAttrSet].data["atretady"]!=undefined && 
					...
					if(Venda.Ebiz.AttributeSwatch.today<etaday){
						ETA = product.attributeValues[eachAttrSet].data["atretamn"]+"/"+product.attributeValues[eachAttrSet].data["atretady"]+"/"+product.attributeValues[eachAttrSet].data["atretayr"];
						jQuery("#addproduct").attr("src",Venda.Ebiz.AttributeSwatch.backorderbt);
						jQuery(".link_btm").append('<p class="eta">* available '+ETA+'</p>');						
						return ;
					}
				}
			}
