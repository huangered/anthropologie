...
function AttributeSwatchClass(){
	...
	this.addFilter = function(attrColumn,attrValue) {
		...
		document.getElementById("alertmessage"+this.productDetailInstance.configObjArea["objProductUniqueId"]).className = "alertMessages normal";
	...
	this.validateAttributes = function(){
		...
		if(this.attrNum==1){
			if(this.filters["att1"]==undefined || this.filters["att1"]==""){
				document.getElementById("alertmessage"+this.productDetailInstance.configObjArea["objProductUniqueId"]).className = "alertMessages warning";
				isSelected = false;
			}
		}else{
			...
			for (var eachAttrSet in this.productInstance.attributeValues) {
				if(this.productInstance.attributeValues[eachAttrSet].values["att1"]==this.filters["att1"]){
					if (this.productInstance.attributeValues[eachAttrSet].data["atrsell"]!="" && this.productInstance.attributeValues[eachAttrSet].data["atrsell"]!=undefined) {
						price = parseFloat(this.productInstance.attributeValues[eachAttrSet].data["atrsell"]).toFixed(2);
					}
					if (this.productInstance.attributeValues[eachAttrSet].data["atrwas"]!="" && this.productInstance.attributeValues[eachAttrSet].data["atrwas"]!=undefined){
						wasprice = parseFloat(this.productInstance.attributeValues[eachAttrSet].data["atrwas"]).toFixed(2);
					}
				}
			}	
	...
	this.backorderbt="bt_comingsoon.gif";
	this.today="";
	
	this.etaDate=function(){
		/* if not has add to basket button exit fn */
		if(jQuery("#addproduct"+this.productDetailInstance.configObjArea["objProductUniqueId"]).length<1) return ;
		/* set media path */
		if(this.mediapath==""){
			...
			for (var eachAttrSet in this.productInstance.attributeValues) {
				if(this.productInstance.attributeValues[eachAttrSet].values["att1"]==this.filters["att1"]){
					if (this.productInstance.attributeValues[eachAttrSet].data["atretady"]!="" && this.productInstance.attributeValues[eachAttrSet].data["atretady"]!=undefined && 
						...
						jQuery("#addproduct"+this.productDetailInstance.configObjArea["objProductUniqueId"] + ", #addproductform"+this.productDetailInstance.configObjArea["objProductUniqueId"] + " .qtymessage").hide();
						jQuery("#addproductform"+this.productDetailInstance.configObjArea["objProductUniqueId"]+" .preorder").show();
						jQuery("#addproductform"+this.productDetailInstance.configObjArea["objProductUniqueId"]+" .link_btm").append('<p class="eta">'+this.availableText+ETA+'</p>');						
							return ;
						}
					}
				}
		...
		jQuery("#addproductform"+this.productDetailInstance.configObjArea["objProductUniqueId"]+" .preorder").hide();
		jQuery("#addproduct"+this.productDetailInstance.configObjArea["objProductUniqueId"] + ", #addproductform"+this.productDetailInstance.configObjArea["objProductUniqueId"] + " .qtymessage").show();
	};
